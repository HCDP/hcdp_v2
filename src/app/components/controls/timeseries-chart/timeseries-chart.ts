...
export class TimeseriesChart {
 ...
 updateOptions = computed<EChartsOption>(() => {
    ...
    const isVisible = this.showDateLine(); 
    const minZoomSpan = this.timeseriesData().period.getHigherOrder()?.valueOf();
    
    const data = this.processedSeriesData();

    const targetMs = targetDate ? targetDate.toMillis() : null;
    const dynamicMarkLine = (isVisible && targetMs) ? {
      symbol: ['none', 'none'],
      animation: false,
      label: {
        show: true,
        formatter: 'Selected Date',
  ...
  constructor() {
    effect(() => {
      ...
      if (rawMap && this.chartInstance) {
        ...
        // before querying internal zoom state
    ...
    effect(() => {
      ...
      const label = this.downloadName();

      const chart = this.chartInstance;
      if(chart) {
        const startStr = this.timeseriesData().formatDate(start);
        const endStr = this.timeseriesData().formatDate(end);
        const fname = `${label}_${startStr}_${endStr}`;
